...
group = ModPrimeSubgroup(modulus, 2)  # ~ group of quadratic residues mod 11
group.set_generator(generator)        # ~ generated by 3: {1, 3, 4, 5, 9}
...
def test_contains(element):
    if element in __residues:
        ...
        assert not group.contains(element)

@pytest.mark.parametrize('element', __elements)
def test_contained_in(element):
    if element in __residues:
        assert element.contained_in(group)
    else:
        assert not element.contained_in(group)
...
def test_decoding(element, decoded):
    assert decoded == group.decode_with_randomness(element)
